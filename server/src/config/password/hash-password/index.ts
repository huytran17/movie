...
export type IHashPassword = ({ password, password_confirmation }: IHashPasswordData) => Promise<string>;

export default function makeHashPassword(hash: IHashFunction): IHashPassword {
  return async function hashPassword({ password, password_confirmation }: IHashPasswordData): Promise<string> {
    if (!password) {
